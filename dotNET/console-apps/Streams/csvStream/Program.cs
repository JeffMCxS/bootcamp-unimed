using static System.Console; //Para não precisar escrever Console no Console.WriteLine() ou Console.ReadLine()
...
static void CriarCsv()
{
    ...
    using var sw = new StreamWriter(path); //Using para dar o flush automatico
...
static void LerCsv()
{
    ...
    if (File.Exists(path))
    {
        ...
        var cabecalho = sr.ReadLine()?.Split(','); //Fazer um separação (split) por virgula, que é como o csv é separado, e guardar em um array
        //ReadLine() por de ser nulo, então ?
...
class Pessoa //Classe no final do arquivo
{
